			...
			<nav class="navbar navbar-expand-lg navbar-light header-site__navbar">
				<div class="container">
					<?php if ( get_theme_mod( 'wp_supercampeones_theme_logo' ) ) : ?>
						<a href="<?php echo home_url('/'); ?>" class="navbar-brand header-site__logo <?php if ( is_front_page() ){ echo 'header-site__logo--home'; } ?>" rel="home" itemprop="url" style="<?php if ( get_theme_mod( 'wp_supercampeones_theme_logo_desktop_width' ) ) { echo 'width: ' . get_theme_mod( 'wp_supercampeones_theme_logo_desktop_width' ) . 'px'; } ?>">
